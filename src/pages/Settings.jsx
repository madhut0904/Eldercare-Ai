...
export default function Settings() {
  const { healthData, liveTicking, setLiveTicking } = useHealth();
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Profile Card */}
        <div className="lg:col-span-5 bg-white rounded-3xl p-6 border border-slate-200/80 card-shadow space-y-5">
          <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
            <div className="w-10 h-10 rounded-xl bg-slate-100 text-slate-700 flex items-center justify-center">
              <User className="w-5 h-5" />
            </div>
            <div>
              <h3 className="font-heading font-extrabold text-lg text-slate-900">Elder Profile</h3>
              <p className="text-xs text-slate-500">Personal & emergency contact details</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSave} className="space-y-4 text-xs">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-slate-900 font-semibold"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-slate-900 font-mono font-semibold"
              />
            </div>
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-200/80 card-shadow space-y-4">
            <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
              ...
              <div>
                <h3 className="font-heading font-extrabold text-lg text-slate-900">Notification Preferences</h3>
                <p className="text-xs text-slate-500">Configure alert delivery channels</p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-3xl p-6 border border-slate-200/80 card-shadow space-y-4">
            <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
              ...
              <div>
                <h3 className="font-heading font-extrabold text-lg text-slate-900">Sensor Telemetry Controls</h3>
                <p className="text-xs text-slate-500">Enable or pause individual sensor modules</p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={item.key} className="flex items-center justify-between p-3 rounded-2xl bg-slate-50 border border-slate-100">
                  ...
                  <button
                    ...
                    onClick={() => handleToggle(item.key)}
                    className={`w-12 h-6 rounded-full transition-colors relative p-0.5 ${
                      toggles[item.key] ? 'bg-emerald-600' : 'bg-slate-300'
                    }`}
                  >
                    <div className={`w-5 h-5 rounded-full bg-white shadow-md transform transition-transform ${
                      toggles[item.key] ? 'translate-x-6' : 'translate-x-0'
              ...
              <div className="flex items-center justify-between p-3 rounded-2xl bg-indigo-50 border border-indigo-100">
                <div>
                  <div className="text-xs font-bold text-indigo-950">Live Micro-Jitter Telemetry Simulator</div>
                  <div className="text-[10px] text-indigo-700">Simulate live heart rate ticking for hackathon demo.</div>
                </div>
                <button
                  type="button"
                  onClick={() => setLiveTicking(!liveTicking)}
                  className={`w-12 h-6 rounded-full transition-colors relative p-0.5 ${
                    liveTicking ? 'bg-indigo-600' : 'bg-slate-300'
                  }`}
                >
                  <div className={`w-5 h-5 rounded-full bg-white shadow-md transform transition-transform ${
