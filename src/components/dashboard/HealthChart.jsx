...
export default function HealthChart() {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200/80 dark:border-slate-800 card-shadow card-shadow-hover transition-colors">
      {/* Header controls: Tabs + Time Range Pill */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <div className="flex items-center gap-2 text-xs font-bold text-slate-400 dark:text-slate-400 uppercase tracking-widest">
        ...
        <div className="flex items-center gap-3 self-start sm:self-auto">
          {/* Time range buttons */}
          <div className="flex items-center p-1 rounded-xl bg-slate-100/90 dark:bg-slate-800 text-xs font-semibold text-slate-600 dark:text-slate-400">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${
              activeTab === tab.id
                ...
                : 'bg-slate-50 text-slate-600 hover:bg-slate-100 dark:bg-slate-800 dark:text-slate-400 hover:dark:bg-slate-700 hover:dark:text-white'
            }`}
          >
            <span>{tab.emoji}</span>
            <span>{tab.label}</span>
      ...
      <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
        ...
        <span className="font-semibold text-slate-700 dark:text-slate-300">99.8% Sensor Fidelity</span>
      </div>
    </div>
  );
}
